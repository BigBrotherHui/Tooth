...
PointSetVtkMapper3D::LocalStorage::LocalStorage()
...
const mitk::PointSet *PointSetVtkMapper3D::GetInput()
{
  return static_cast<const mitk::PointSet *>(GetDataNode()->GetData());
}

PointSetVtkMapper3D::PointSetVtkMapper3D()
    : 
...
//void PointSetVtkMapper3D::ReleaseGraphicsResources(vtkWindow *renWin)
//{
...
void PointSetVtkMapper3D::ReleaseGraphicsResources(mitk::BaseRenderer *renderer)
{
...
void PointSetVtkMapper3D::CreateVTKRenderObjects(mitk::BaseRenderer* renderer)
{
  LocalStorage* ls = m_LSH.GetLocalStorage(renderer);
  ...
  {
    if (this->GetDataNode()->GetIntProperty("BaseRendererMapperID", mapperID) && mapperID == mitk::BaseRenderer::Standard3D)
...
void PointSetVtkMapper3D::GenerateDataForRenderer(mitk::BaseRenderer *renderer)
{
  LocalStorage* ls = m_LSH.GetLocalStorage(renderer);
  bool visible = true;
...
void PointSetVtkMapper3D::ResetMapper(mitk::BaseRenderer * renderer)
{
    LocalStorage* ls = m_LSH.GetLocalStorage(renderer);
  ls->m_PointsAssembly->VisibilityOff();
}

vtkProp *PointSetVtkMapper3D::GetVtkProp(mitk::BaseRenderer * renderer)
{
    LocalStorage* ls = m_LSH.GetLocalStorage(renderer);
  return ls->m_PointsAssembly;
}

void PointSetVtkMapper3D::UpdateVtkTransform(mitk::BaseRenderer * /*renderer*/)
{
}

void PointSetVtkMapper3D::ApplyAllProperties(mitk::BaseRenderer *renderer, vtkActor *actor)
{
    LocalStorage* ls = m_LSH.GetLocalStorage(renderer);
...
void PointSetVtkMapper3D::CreateContour(vtkPoints* points, vtkCellArray* PointConnections, mitk::BaseRenderer* renderer)
{
    LocalStorage* ls = m_LSH.GetLocalStorage(renderer);
...
void PointSetVtkMapper3D::SetDefaultProperties(mitk::DataNode *node, mitk::BaseRenderer *renderer, bool overwrite)
{
